	...
	methods: {
		...
		fetchData: function(url) {
			...
			$.get(
				...
				function(data) {
					if (data.count) {
						this.all_categories = _.sortBy(
							_.uniq(this.categories.concat(data.categories), "id"),
							"position"
						);
						this.sections_and_subsections = _.sortBy(
							_.uniq(this.sections.concat(data.sections), "id"),
							"position"
						);

						this.subsections = _.filter(this.sections_and_subsections, function(
							section
						) {
							return section.parent_section_id !== null;
						});
						this.sections = _.filter(this.sections_and_subsections, function(
							section
						) {
							return section.parent_section_id === null;
						});
						this.categories = _.filter(this.all_categories, function(category) {
							return category.name !== "Contact page";
						});
